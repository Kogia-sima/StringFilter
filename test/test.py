...
def test_rabbit_and_turtle() -> None:
    ...
    assert sf.is_valid('rabbit vs turtle')
    assert not sf.is_valid('Sea turtles were released to the ocean.')


def test_null_str() -> None:
    sf = StringFilter.StringFilter('')
    assert sf.is_valid('Buffalo buffalo Buffalo buffalo buffalo buffalo Buffalo buffalo.')
